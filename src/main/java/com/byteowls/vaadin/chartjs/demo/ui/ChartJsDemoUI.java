...
@Theme("chartjs")
@SpringUI(path = "/chartjs")
public class ChartJsDemoUI extends UI {

    private static final long serialVersionUID = -33887281222947647L;
    private com.vaadin.external.org.slf4j.Logger log = LoggerFactory.getLogger(ChartJsDemoUI.class);
...
//        MENU_ITEMS.add(new MenuItem(ChartType.AREA, "Browser", BrowserView.class));
    }
    ...
    public String getFormattedSourceCode(String sourceCode) {
        if (sourceCode != null) {
            ...
            } catch (IllegalConfigurationException ignore) {
                ignore.printStackTrace();
            } catch (IOException ex) {
                Logger.getLogger(ChartJsDemoUI.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
            log.info("source is " + sourceCode);
        }
        return sourceCode;
    }
